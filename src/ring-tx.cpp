...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " ring-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  ring-tx [options] <hex-tx> [commands]  Update hex-encoded ring transaction\n" +
            "or:     ring-tx [options] -create [commands]   Create hex-encoded ring transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ring transaction
