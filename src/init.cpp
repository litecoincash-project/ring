 ...
 */
static const char* RING_PID_FILENAME = "ringd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", RING_PID_FILENAME)));
}
 ...
 * between ringd, and ring-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", RING_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
/** Sanity checks
 *  Ensure that Ring is running in a usable environment with all
...
{
    // Make sure only a single Ring process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", RING_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if ring is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ring is started while in a temporary directory.\n",
