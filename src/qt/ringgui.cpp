...
RingGUI::RingGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &RingGUI::showModalOverlay);
        connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &RingGUI::showModalOverlay);
        connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &RingGUI::showModalOverlay);
    }
#endif
...
void RingGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Ring address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ring: URIs)"));
    ...
    connect(receiveCoinsAction, &QAction::triggered, this, &RingGUI::gotoReceiveCoinsPage);
    connect(receiveCoinsMenuAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsMenuAction, &QAction::triggered, this, &RingGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &RingGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Ring addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Ring addresses"));
    ...
    openAction->setStatusTip(tr("Open a ring: URI or payment request"));
    ...
    connect(optionsAction, &QAction::triggered, this, &RingGUI::optionsClicked);
    connect(toggleHideAction, &QAction::triggered, this, &RingGUI::toggleHidden);
    connect(showHelpMessageAction, &QAction::triggered, this, &RingGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &RingGUI::showDebugWindow);
        ...
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& path : wallets) {
                ...
                connect(action, &QAction::triggered, [this, name, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &RingGUI::setCurrentWallet);
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_C), this), &QShortcut::activated, this, &RingGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_D), this), &QShortcut::activated, this, &RingGUI::showDebugWindow);
}

void RingGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void RingGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &RingGUI::setCurrentWalletBySelectorIndex);
...
void RingGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    if(_clientModel)
    {
        ...
        updateNetworkState();
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &RingGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &RingGUI::setNetworkActive);

        modalOverlay->setKnownBestHeight(_clientModel->getHeaderTipHeight(), QDateTime::fromTime_t(_clientModel->getHeaderTipTime()));
        setNumBlocks(m_node.getNumBlocks(), QDateTime::fromTime_t(m_node.getLastBlockTime()), m_node.getVerificationProgress(), false);
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &RingGUI::setNumBlocks);
        ...
        if (optionsModel && trayIcon) {
            ...
            connect(optionsModel, &OptionsModel::hideTrayIconChanged, this, &RingGUI::setTrayIconVisible);
...
#ifdef ENABLE_WALLET
void RingGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    connect(wallet_controller, &WalletController::walletAdded, this, &RingGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &RingGUI::removeWallet);
...
void RingGUI::addWallet(WalletModel* walletModel)
{
...
void RingGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void RingGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void RingGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void RingGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void RingGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &RingGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler *dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &RingGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void RingGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void RingGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void RingGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void RingGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void RingGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void RingGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void RingGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void RingGUI::openClicked()
{
...
void RingGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RingGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RingGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RingGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void RingGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Ring network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void RingGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void RingGUI::updateHeadersSyncProgressLabel()
{
...
void RingGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
...
void RingGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
...
void RingGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Ring"); // default title
...
void RingGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &RingGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &RingGUI::show);
                e->ignore();
            }
        }
    }
...
void RingGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RingGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RingGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(RingUnits::formatWithUnit(unit, amount, true));
...
void RingGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RingGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RingGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RingGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RingGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
    ...
    case WalletModel::Unlocked:
        ...
        if(fWalletUnlockHiveMiningOnly) 
            labelWalletEncryptionIcon->setToolTip(tr("Wallet is <b>encrypted</b> and currently <b>unlocked for hive and PoP</b>"));    
        else
...
void RingGUI::updateWalletStatus()
{
    if (!walletFrame) {
        return;
    }
...
void RingGUI::updateWindowTitle()
{
...
void RingGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RingGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RingGUI::detectShutdown()
{
...
void RingGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void RingGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RingGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void RingGUI::subscribeToCoreSignals()
{
...
void RingGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(nullptr)
{
    ...
    QList<RingUnits::Unit> units = RingUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const RingUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(RingUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const RingUnits::Unit u : RingUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(RingUnits::longName(u)), this);
...
{
    setText(RingUnits::longName(newUnits));
}
