...
###   ringcoin.tech and ring.org and compares them.
...
WORKINGDIR="/tmp/ring_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ringcoin.tech"
HOST2="https://ring.org"
BASEDIR="/bin/"
VERSIONPREFIX="ring-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ring-',
...
if [ $? -ne 0 ]; then
   echo "ring.org failed to provide signature file, but ringcoin.tech did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ring.org and ringcoin.tech signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ring Core binary release signing key installed?"
   fi
