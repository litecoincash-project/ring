...
###   ringcore.org and ring.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "ring.org failed to provide signature file, but ringcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ring.org and ringcore.org signature files were not equal?"
